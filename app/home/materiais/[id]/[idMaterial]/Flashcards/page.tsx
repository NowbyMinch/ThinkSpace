...
export default function MaterialClient() {
  ...
  const [flashcards, setFlashcards] = useState<FlashcardsApiResponse>({
    flashcards: [],
  });
  ...
  useEffect(() => {
    const Flashcards = async (id: string) => {
      try {
        if (origem === "DOCUMENTO") {
          const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/materiais/flashcards-pdf/${id}`,
            {
              method: "GET",
              credentials: "include",
            }
          ...
          const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/materiais/flashcards/${id}`,
            {
              method: "GET",
              credentials: "include",
            }
      ...
      <div className="relative w-full rounded-[35px] bg-white h-full flex flex-col items-center shadow-md border border-[#00000031] ">
        <div className=" w-full rounded-[35px] overflow-hidden bg-white h-full flex flex-col items-center shadow-md border border-[#00000031]">
          <div className="mt-[25px] mb-[25px] overflow-hidden w-[1200px] max-w-[90%] h-[950px] max-h-[95%] flex flex-col justify-center gap-4 items-center ">
            ...
            <div className="w-full gap-2 h-[80%] bg-[#f0f0fb] rounded-[25px] px-1 border-[2px] shadow-md border-[rgba(60,49,91,0.24)] flex justify-center items-center perspective relative">
              <motion.div
              ...
              <div className="relative w-[80%] h-[85%] gap-1 cursor-pointer flex flex-col justify-center items-center ">
                ...
                >
                        ...
                        <div className="w-[95%] h-[90%] flex justify-center items-center relative rounded-[25px] bg-white overflow-hidden">
                          ...
                          <div className="w-[70%] h-[80%] flex flex-col justify-end items-center ">
                            <h1 className="text-[35px] text-center">
                              Parabéns! Você completou os flashcards e ganhou
                              mais 5XP pela dedicação!
                            </h1>
                            <div className="w-[170px]">
                              <Image
                                width={300}
                                height={500}
                                src="/concluiu-medalha.svg"
                                className="w-full "
                                alt="Logo"
                              />
                            </div>
                  ...
                  <div
                    className="absolute w-full h-full backface-hidden text-center bg-white rounded-[25px] flex items-center justify-center p-6 text-xl font-medium shadow-md"
                ...
                <h1 className="text-[30px]">
                  {questaoIndex + 1}/{flashcards.flashcards.length}
                </h1>
              </div>
              <motion.div
                whileHover={{ scale: 1.03 }}
                ...
                onClick={() => {
                  ...
                  if (questaoIndex + 1 !== flashcards.flashcards.length) {
                    if (flipped) {
                      setFlipped(false); // flip back to question

                      // wait for flip animation to finish (0.6s)
                      setTimeout(() => {
                        if (questaoIndex + 1 < flashcards.flashcards.length) {
                ...
                <div className="flex gap-2 max-w-[95%] h-[100px] max-h-[90%] justify-center items-center ">
                  <motion.button
                    ...
                    onClick={() => {
                      ...
                      if (questaoIndex !== flashcards.flashcards.length - 1) {
                        setQuestaoIndex(questaoIndex + 1);
                      } else {
                        return;
                      }
