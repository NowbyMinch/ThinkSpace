...
export const ChatMateriais = ({ idMaterial, appear }: ChatMateriaisProps) => {
  ...
  const Mensagem = async (texto: string) => {
    ...
    setHistoricoUsuario((prev) => [
      ...prev,
      { id: tempId, mensagemUsuario: texto },
    ]);
    ...
    <AnimatePresence>
      {appear && (
        <>
          <div
            ref={parentRef}
            className="w-full h-full rounded-[35px] pointer-events-none absolute left-0 right-0 top-0 bottom-0"
          ></div>
          ...
          {position && (
            ...
            >
              ...
              <div className="text-gray-400 py-1 px-2 text-center ">
                Para manter a experiência leve e segura, suas conversas ficam
                salvas por até 30 dias.
              </div>
