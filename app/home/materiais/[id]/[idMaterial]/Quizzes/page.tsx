...
export default function MaterialClient() {
  ...
  let acertou = 0;
  const [estado, setEstado] = useState<Final | null>(null);
  ...
  const final = async (): Promise<Final> => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/materiais/respostas-quiz/${idMaterial}`,
        {
          method: "GET",
          credentials: "include",
        }
  ...
  useEffect(() => {
    const quizzes = async (id: string) => {
      try {
        if (origem === "DOCUMENTO") {
          const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/materiais/quizzes-pdf/${id}`,
            {
              method: "GET",
              credentials: "include",
            }
          ...
          const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/materiais/quizzes/${id}`,
            {
              method: "GET",
              credentials: "include",
            }
  ...
  const questao = async (indice: number) => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/materiais/responder-questao/${idMaterial}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      ...
      <div
        ref={parentRef}
        className="relative w-full rounded-[35px] bg-white h-full flex flex-col items-center shadow-md border border-[#00000031] "
      >
          ...
          <div className="w-[1200px] max-w-[90%] h-[850px] max-h-[95%] my-auto relative flex flex-col justify-center items-center gap-1">
            <div className="w-full h-[10px] rounded-full bg-[rgba(16,19,46,0.14)] absolute top-0 ">
              <div
                ...
                className="transition-all ease-in-out duration-300 h-full rounded-full bg-[rgba(30,35,81,0.75)]"
              ></div>
            </div>
            ...
            {(() => {
              if (questaoIndex == 10) {
                return (
                  <h2 className="mr-auto text-[15px] bg-[#A39CEC] py-1 px-2 rounded-[10px] text-white">
                    Questão {questaoIndex}/{quizzes.length}
                  </h2>
                );
              } else {
                return (
                  <h2 className="mr-auto text-[15px] bg-[#A39CEC] py-1 px-2 rounded-[10px] text-white">
                    Questão {questaoIndex + 1}/{quizzes.length}
                  </h2>
                );
              }
            })()}
            ...
            <div className="w-full h-[85%] bg-[#F7F7FF] rounded-[25px] border-[2px] shadow-md border-[rgba(60,49,91,0.24)] flex justify-center items-center relative overflow-y-auto overflow-x-hidden ">
              <div className="w-[85%] max-w-[90%] h-[90%] flex justify-center items-center relative ">
                <div className="w-full h-full flex flex-col items-center relative gap-2">
                  <h1 className="quizz_title text-[20px] min-h-fit text-center line-clamp-7 break-words ">
                    {quizzes[questaoIndex]?.pergunta}
                  </h1>
                          ...
                          <div className="w-[95%] h-[90%] flex justify-center items-center relative rounded-[25px] bg-white overflow-hidden">
                            ...
                            <div className="w-[70%] h-[80%] flex flex-col justify-center items-center ">
                              <h1 className="text-[35px] text-center">
                                Parabéns! Você completou o quizz e ganhou mais{" "}
                                {xp?.xp}XP. Revise suas respostas e continue
                                subindo no ranking.
                              </h1>
                            </div>
                          </div>
